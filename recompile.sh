...
docker exec -ti blog_web_1 sh -c "kill $(ps aux | grep 'bin/Debug/netcoreapp2.1/Blog.Web.dll' | awk '{print $2}') || true"
#Copy 
echo "copy files"
...
#Dotnet restore
docker exec -ti blog_web_1 sh -c "cd Blog.Web && dotnet restore && dotnet build"
#Run
